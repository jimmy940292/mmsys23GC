...
public:
    ...
    void SortSession(std::multimap<Duration, fw::shared_ptr<SessionStreamController>> &sortmmap) override
    {
        ...
        // Sort by RTT
        std::multimap<Duration, fw::shared_ptr<SessionStreamController>> new_queue, weighted_queue;
        for (auto &&sessionItor : m_dlsessionmap)
        {
            auto score = sessionItor.second->GetRtt();
            new_queue.emplace(score, sessionItor.second);
            // sortmmap.emplace(score, sessionItor.second);
        }
        

        // Convert score to weights: divide the smallest score by each score
        std::multimap<Duration, fw::shared_ptr<SessionStreamController>>::iterator temp_it = new_queue.end();
        auto max_rtt = (*temp_it).first;
        ...
        for (size_t i = weighted_queue.size(); i > 0; i--)
        {
            std::multimap<Duration, fw::shared_ptr<SessionStreamController>>::iterator it = weighted_queue.begin();
            auto weight = (*it).first;
            for(size_t j = 0; j < i; j++, ++it)
            {
                sortmmap.emplace((*it).second->GetRtt(), (*it).second);
            }
        }
    }
