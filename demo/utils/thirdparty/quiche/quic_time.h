...
/// we use std::chrono::microsecond as delta type
#pragma once
...
// QuicWallTime, below.
        ...
        public:
            ...
            public:
                ...
                static constexpr Delta Infinite()
                {
                ...
                static constexpr Delta FromMicroseconds(int64_t us)
                {
                    return Delta(us);
                }

                // Converts from std::chrono::microseconds.
                static Delta FromStdDuration(std::chrono::microseconds duration)
                {
                    if (duration == std::chrono::microseconds::max())
                    {
                        return Infinite();
                    }
                    return Delta(duration.count());
                }
                ...
                std::chrono::microseconds ToAbsl()
                {
                    if (IsInfinite())
                    {
                        return std::chrono::microseconds::max();
                    }
                    return std::chrono::microseconds(time_offset_);
                }

                constexpr bool IsZero() const
                {
                    return time_offset_ == 0;
                }
            ...
            static constexpr QuicTime Zero()
            {
                return QuicTime(0);
            }
            ...
            int64_t ToDebuggingValue() const
            {
                return time_;
            }
...
// requires some flexibility.
        ...
        public:
            ...
            // value.
            static constexpr QuicWallTime Zero()
            {
